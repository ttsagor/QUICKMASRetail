...
import com.quickmas.salim.quickmasretail.Module.RTM.POSM.POSMSale.Adaptar_crm_invoice_company;
...
public class PosmCurrentInvoice extends AppCompatActivity {
    DBInitialization db;
    int amountCount=0;
    int uPriceCount=0;
    ...
    ArrayList<POSMInvoice> invoices = new ArrayList<>();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        actionBar.hide();
        setContentView(R.layout.activity_posm_current_invoice);
        mContex = this;
        ...
        headtext.setText(DashboardSubMenu.getMenuText(db, "rtm_activity_4"));
        ...
        qnty = setTextFont(qnty, this,db,"Quantity");
        ...
        description = setTextFont(description, this,db,"Description");
        ...
        //DebugHelper.print(POSMInvoiceHead.select(db,"1=1"));
        //DebugHelper.print(POSMInvoice.select(db,"1=1"));

        invoices = POSMInvoice.getPendingInvoices(db);

        POSMInvoiceHead crmInvoiceHead = POSMInvoiceHead.getPendingInvoices(db).get(0);


        String invoice = invoices.get(0).getPrefix() + String.valueOf(crmInvoiceHead.getOutlet());
        ...
        for(POSMInvoice cInvoice : invoices)
        {
            Product product = Product.select(db,db.COLUMN_product_id+"="+cInvoice.getProduct_id()).get(0);
            product.setSub_sku(cInvoice.getRemark());
            product.setBrand(cInvoice.getDetails());
            product.setSku_qty(cInvoice.getProduct_quantity());
        ...
        sell_invoice_add_new.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                Intent i = new Intent( PosmCurrentInvoice.this ,PosmProductSelect.class);
                    ...
                    public void onClick(final DialogInterface dialog, int which) {
                        switch (which){
                            case DialogInterface.BUTTON_POSITIVE:
                                ...
                                final ProgressDialog progressDoalog = new ProgressDialog(PosmCurrentInvoice.this);
                                    ...
                                    public void run() {
                                        ...
                                        sell_invoice_date.post(new Runnable() {
                                            public void run() {
                                                ...
                                                Intent intent = new Intent(PosmCurrentInvoice.this,POSMList.class);
        ...
        sell_invoice_discard.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                AlertDialog.Builder builder = new AlertDialog.Builder(PosmCurrentInvoice.this);
                ...
                builder.setPositiveButton(textSelectByVarname(db,"sellInvoice_sell_invoice_dialog_yes").getText(), new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialog, int which) {
                        ...
                        POSMInvoice.deletePendingInvoices(db);
                        POSMInvoiceHead.deletePendingInvoices(db);
                        Intent intent = new Intent(PosmCurrentInvoice.this,MainActivity.class);
                        intent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                        startActivity(intent);
                        dialog.dismiss();
                        finish();
        ...
        if(invoices.size()>0)
        {
            ...
            for (int i = 0; i < image_list.size(); i++) {
                try
                {
                    ...
                    String root = Environment.getExternalStorageDirectory().toString() + "/" + FileManagerSetting.folder_name + "/" + FileManagerSetting.rtm_posm_product + "/" + fileName;
            ...
            int quantity = 0;
            for(POSMInvoice invoice : invoices)
            {
                ...
                invoice.setStatus(1);
                quantity+=invoice.getProduct_quantity();
                invoice.insert(db);
            }

            POSMInvoiceHead invoiceHead = POSMInvoiceHead.getPendingInvoices(db).get(0);
