...
import com.quickmas.salim.quickmasretail.Module.RTM.FreeProduct.FreeProductSale.Adaptar_invoice_company;
...
public class FreeProductPrint extends AppCompatActivity {
    DBInitialization db;
    int amountCount=0;
    int uPriceCount=0;
    ...
    protected void onCreate(Bundle savedInstanceState) {
       ...
       // setContentView(R.layout.activity_free_product_print);
        ...
        headtext.setText(DashboardSubMenu.getMenuText(db, "rtm_activity_list_2"));
        ...
        invoices = FreeProductInvoice.select(db,db.COLUMN_free_Invoice_invoice_id+"="+in_invoice);

        FreeProductInvoiceHead freeProductInvoiceHead = FreeProductInvoiceHead.select(db,db.COLUMN_free_Invoice_head_id+"="+in_invoice).get(0);
        ...
        for(String img : freeProductInvoiceHead.getPhoto().split(";"))
        {
            image_list.add(Uri.parse(Environment.getExternalStorageDirectory().toString() + "/" + FileManagerSetting.folder_name + "/" + FileManagerSetting.rtm_free_product + "/" + img.trim()));
        }
        createImageList();
    }
