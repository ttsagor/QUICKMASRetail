package com.quickmas.salim.quickmasretail.Module.RTM.POSM.POSMPrint;
...
public class POSMPrint extends AppCompatActivity {
    DBInitialization db;
    int amountCount=0;
    int uPriceCount=0;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //setContentView(R.layout.activity_posmprint);
        ...
        invoices = POSMInvoice.select(db,db.COLUMN_crm_Invoice_invoice_id+"="+in_invoice);

        POSMInvoiceHead invoiceHead = POSMInvoiceHead.select(db,db.COLUMN_posm_Invoice_head_id+"="+in_invoice).get(0);


        String invoice = invoices.get(0).getPrefix() + String.valueOf(invoices.get(0).getInvoice_id());

        sell_invoice_outlet = setTextFont(sell_invoice_outlet, this, invoiceHead.getOutlet());
        ...
        for(POSMInvoice cInvoice : invoices)
        {
            ...
            gqnty+=cInvoice.getProduct_quantity();
        ...
        for(String img : invoiceHead.getPhoto().split(";"))
        {
            image_list.add(Uri.parse(Environment.getExternalStorageDirectory().toString() + "/" + FileManagerSetting.folder_name + "/" + FileManagerSetting.rtm_posm_product + "/" + img.trim()));
        }
        ...
        total_qnty = setTextFont(total_qnty,this,String.valueOf(gqnty));
