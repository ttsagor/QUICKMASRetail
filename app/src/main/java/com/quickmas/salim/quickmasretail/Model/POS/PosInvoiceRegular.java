...
import static com.quickmas.salim.quickmasretail.Database.DBInitialization.TABLE_PosInvoice_regular;
...
public class PosInvoiceRegular{
        ...
        public void insert(DBInitialization db)
        {
            ...
            db.insertData(getContectValue(db), TABLE_PosInvoice_regular,con);
        }

        public void update(DBInitialization db)
        {
            db.updateData(getContectValue(db), TABLE_PosInvoice_regular,db.COLUMN_pos_invoice_id+"="+this.getId());
        }
        public static void update(DBInitialization db,String data,String con)
        {
            db.updateData(data,con, TABLE_PosInvoice_regular);
        }

        public static int count(DBInitialization db, String con)
        {
            return db. getDataCount(TABLE_PosInvoice_regular,con);
        }

        public static int sumData(DBInitialization db,String sel, String con)
        {
            return db.sumData(sel, TABLE_PosInvoice_regular, con);
        }

        public static ArrayList<PosInvoiceRegular> getPendingInvoices(DBInitialization db)
        {
            return select(db, COLUMN_pos_invoice_status+"=0");
        }
        public static ArrayList<PosInvoiceRegular> getPaymentPendingInvoices(DBInitialization db)
        {
            return select(db, COLUMN_pos_invoice_status+"=1 OR "+COLUMN_pos_invoice_status+"=0");
        }
        public static ArrayList<PosInvoiceRegular> select(DBInitialization db, String con)
        {
            Cursor c = db.getData("*",db.TABLE_PosInvoice_regular,con,"");
            ArrayList<PosInvoiceRegular> invoices = new ArrayList<PosInvoiceRegular>();
        ...
        {
            Cursor c = db.getData("*",TABLE_PosInvoice_regular,db.COLUMN_pos_invoice_status+"!=0"+" OR "+db.COLUMN_pos_invoice_status+"!=4",db.COLUMN_pos_invoice_invoiceID);
            return c.getCount();
        }

        public static void deletePendingInvoices(DBInitialization db,int id)
        {
            db.deleteData( TABLE_PosInvoice_regular ,COLUMN_pos_invoice_status+"=0 AND "+COLUMN_pos_invoice_invoiceID+"="+id);
        }
        ...
        {
            db.updateData(COLUMN_pos_invoice_status+"="+newS,COLUMN_pos_invoice_invoiceID+"="+String.valueOf(invoiceID), TABLE_PosInvoice_regular);
        }

        public static void updateInvoiceStatus(DBInitialization db,String newS)
        {
            db.updateData(COLUMN_pos_invoice_status+"="+newS,COLUMN_pos_invoice_status+"=0", TABLE_PosInvoice_regular);
        }

        public static void deleteIteamExchange(DBInitialization db)
        {
            db.deleteData( TABLE_PosInvoice_regular ,COLUMN_pos_invoice_status+"=9");
        }

        public static void deleteIteamFromPendingList(DBInitialization db,int invoiceID,int product)
        {
            db.deleteData( TABLE_PosInvoice_regular ,COLUMN_pos_invoice_status+"=0 AND "+COLUMN_pos_invoice_invoiceID+"="+invoiceID +" AND "+COLUMN_pos_invoice_productId+"="+product);
        }
        ...
        {
            ArrayList<PosInvoiceRegular> inv = PosInvoiceRegular.select(db, db.COLUMN_pos_invoice_status + "=0");
            if(inv.size()>0)
            {
                return inv.get(0).getInvoiceID();
            }
            else
            {
                return db.getMax(TABLE_PosInvoice_regular,COLUMN_pos_invoice_invoiceID, db.COLUMN_pos_invoice_status + "!=0")+1;
            }
        }
        public static int getNewInvoice(DBInitialization db)
        {
            return db.getMax(TABLE_PosInvoice_regular,COLUMN_pos_invoice_invoiceID,  "1=1");
        }
