...
public class PosInvoiceHeadWeb {
    public int id;
    ...
    public void insert(DBInitialization db)
    {
        ...
        db.insertData(getContectValue(db), TABLE_pos_invoice_head_web,con);
    }

    public void update(DBInitialization db)
    {
        db.updateData(getContectValue(db), TABLE_pos_invoice_head_web, COLUMN_pos_invoice_head_id_web+"="+this.getId());
    }
    public static void update(DBInitialization db,String data,String con)
    {
        db.updateData(data,con, TABLE_pos_invoice_head_web);
    }

    public static ArrayList<PosInvoiceHeadWeb> select(DBInitialization db, String con)
    {
        Cursor c = db.getData("*", TABLE_pos_invoice_head_web,con,"");
        ArrayList<PosInvoiceHeadWeb> invoices = new ArrayList<PosInvoiceHeadWeb>();
        if (c !=null && c.getCount() > 0) {
            c.moveToFirst();
            do {
                PosInvoiceHeadWeb invoice = new PosInvoiceHeadWeb();
    ...
    {
        return db.getDataCount(TABLE_pos_invoice_head_web,"1=1");
    }
