...
public class CashPaymentReceive {
    ...
    ContentValues getContectValue(DBInitialization db)
    {
        ...
        if(this.getId()>0)
        {
            insertValues.put(COLUMN_cash_payemnt_id, this.getId());
        }
        insertValues.put(db.COLUMN_cash_payemnt_account_name, this.getAccount_name());
        insertValues.put(db.COLUMN_cash_payemnt_remark, this.getRemark());
        insertValues.put(db.COLUMN_cash_payemnt_ca_amount, this.getCa_amount());
        insertValues.put(db.COLUMN_cash_payemnt_cash_amount, this.getCash_amount());
        insertValues.put(db.COLUMN_cash_payemnt_card_amount, this.getCard_amount());
        insertValues.put(db.COLUMN_cash_payemnt_card_type, this.getCard_type());
        insertValues.put(db.COLUMN_cash_payemnt_bank_account, this.getBank_account());
        insertValues.put(db.COLUMN_cash_payemnt_bank_name, this.getBank_name());
        insertValues.put(db.COLUMN_cash_payemnt_customer_name, this.getCustomer_name());
        insertValues.put(db.COLUMN_cash_payemnt_cheque_no, this.getCheque_no());
        insertValues.put(db.COLUMN_cash_payemnt_received_by, this.getReceived_by());
        insertValues.put(db.COLUMN_cash_payemnt_date_time, this.getDate_time());
        insertValues.put(db.COLUMN_cash_payemnt_date_time, this.getDate_time());
        insertValues.put(db.COLUMN_cash_payemnt_data_synced, this.getIf_data_synced());
        insertValues.put(db.COLUMN_cash_payemnt_voucher_no, this.getVoucher());
        insertValues.put(db.COLUMN_cash_payemnt_agent, this.getAgent());
        return  insertValues;
    }
    public void insert(DBInitialization db,String con)
    {
        db.insertData(getContectValue(db), TABLE_cash_payment,con);
    }
    public void insert(DBInitialization db)
    {
        String con = COLUMN_cash_payemnt_id+"="+this.getId();
        db.insertData(getContectValue(db), TABLE_cash_payment,con);
    }
    public void update(DBInitialization db)
    {
        db.updateData(getContectValue(db), TABLE_cash_payment, COLUMN_cash_payemnt_id+"="+this.getId());
    }
    public static void update(DBInitialization db,String data,String con)
    {
        db.updateData(data,con, TABLE_cash_payment);
    }
    public static int getCount(DBInitialization db)
    {
        return db.getDataCount(TABLE_cash_payment,"1=1");
    }

    public static ArrayList<CashPaymentReceive> select(DBInitialization db, String con)
    {
        Cursor c = db.getData("*", TABLE_cash_payment,con,"");
        ArrayList<CashPaymentReceive> cashPaymentReceives = new ArrayList<CashPaymentReceive>();
        if (c !=null && c.getCount() > 0) {
            c.moveToFirst();
            do {
                CashPaymentReceive cashPaymentReceive = new CashPaymentReceive();
                cashPaymentReceive.setId(c.getInt(c.getColumnIndex(COLUMN_cash_payemnt_id)));
                cashPaymentReceive.setAccount_name(c.getString(c.getColumnIndex(db.COLUMN_cash_payemnt_account_name)));
                cashPaymentReceive.setRemark(c.getString(c.getColumnIndex(db.COLUMN_cash_payemnt_remark)));
                cashPaymentReceive.setCa_amount(c.getDouble(c.getColumnIndex(db.COLUMN_cash_payemnt_ca_amount)));
                cashPaymentReceive.setCash_amount(c.getDouble(c.getColumnIndex(db.COLUMN_cash_payemnt_cash_amount)));
                cashPaymentReceive.setCard_amount(c.getDouble(c.getColumnIndex(db.COLUMN_cash_payemnt_card_amount)));
                cashPaymentReceive.setCard_type(c.getString(c.getColumnIndex(db.COLUMN_cash_payemnt_card_type)));
                cashPaymentReceive.setBank_account(c.getString(c.getColumnIndex(db.COLUMN_cash_payemnt_bank_account)));
                cashPaymentReceive.setBank_name(c.getString(c.getColumnIndex(db.COLUMN_cash_payemnt_bank_name)));
                cashPaymentReceive.setCheque_no(c.getString(c.getColumnIndex(db.COLUMN_cash_payemnt_cheque_no)));
                cashPaymentReceive.setCustomer_name(c.getString(c.getColumnIndex(db.COLUMN_cash_payemnt_customer_name)));
                cashPaymentReceive.setReceived_by(c.getString(c.getColumnIndex(db.COLUMN_cash_payemnt_received_by)));
                cashPaymentReceive.setDate_time(c.getString(c.getColumnIndex(db.COLUMN_cash_payemnt_date_time)));
                cashPaymentReceive.setIf_data_synced(c.getInt(c.getColumnIndex(db.COLUMN_cash_payemnt_data_synced)));
                cashPaymentReceive.setVoucher(c.getString(c.getColumnIndex(db.COLUMN_cash_payemnt_voucher_no)));
                cashPaymentReceive.setAgent(c.getString(c.getColumnIndex(db.COLUMN_cash_payemnt_agent)));
    ...
    {
        return db.getMax(TABLE_cash_payment,COLUMN_cash_payemnt_id, "1=1");
    }
