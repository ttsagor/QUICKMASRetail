...
public class PosInvoiceHead {
    ...
    public void insert(DBInitialization db)
    {
        ...
        db.insertData(getContectValue(db), TABLE_pos_invoice_head,con);
    }

    public void update(DBInitialization db)
    {
        db.updateData(getContectValue(db), TABLE_pos_invoice_head,db.COLUMN_pos_invoice_head_id+"="+this.getId());
    }
    public static void update(DBInitialization db,String data,String con)
    {
        db.updateData(data,con, TABLE_pos_invoice_head);
    }

    public static ArrayList<PosInvoiceHead> select(DBInitialization db, String con)
    {
        Cursor c = db.getData("*", TABLE_pos_invoice_head,con,"");
        ArrayList<PosInvoiceHead> invoices = new ArrayList<PosInvoiceHead>();
        if (c !=null && c.getCount() > 0) {
            c.moveToFirst();
            do {
                PosInvoiceHead invoice = new PosInvoiceHead();
    ...
    {
        return db.getDataCount(TABLE_pos_invoice_head,"1=1");
    }
