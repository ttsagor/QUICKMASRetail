...
public class PosInvoicePrint extends AppCompatActivity {
    ...
    ArrayList<PosInvoiceCombian> posInvoice = new ArrayList<>();
    Context mContex;
    ...
    protected void onCreate(Bundle savedInstanceState) {
       ...
       // DebugHelper.print(cUser);
        ...
        {
            posInvoiceHead =  PosInvoiceHeadCombian.getPosInvoiceCombianInvoiceWeb(PosInvoiceHeadWeb.select(db,db.COLUMN_pos_invoice_head_id_web+"="+id)).get(0);
            posInvoice = PosInvoiceCombian.setPosInvoiceCombianWeb(PosInvoiceWeb.select(db,db.COLUMN_pos_invoice_invoiceID_web+"="+id));
        }
        else
        {
           ...
           posInvoice = PosInvoiceCombian.setPosInvoiceCombian(PosInvoice.select(db,db.COLUMN_pos_invoice_invoiceID+"="+id));
        }
        ...
        TextView inv_number = FontSettings.setTextFont((TextView) findViewById(R.id.inv_number),this,"Invoice Number: "+cUser.getSelected_pos()+"-"+category+"-"+posInvoiceHead.getInvoice_id());
        ...
        sell_invoice_home.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                Intent intent = new Intent(PosInvoicePrint.this, com.quickmas.salim.quickmasretail.Module.Dashboard.MainActivity.class);
    ...
    public void productListShow(ViewData data) {
        PosInvoiceCombian posInvoice = (PosInvoiceCombian) data.object;
        ...
        Double free = 0.0;
        try{
            title = PosProduct.select(db,db.COLUMN_pos_product_name+"=\""+posInvoice.getProductName()+"\"").get(0).getTitle();
        }catch (Exception e){
    ...
    public static void printInvoice(DBInitialization db, Context mContex,Button summary_print,String category,String id)
    {
        ...
        printData+="Invoice Number: "+cUser.getSelected_pos()+"-"+category+"-"+posInvoiceHead.getInvoice_id()+"\n";
        ...
        for(PosInvoiceCombian invoice: posInvoice)
        {
            ...
            Double free = 0.0;
            try{
                title = PosProduct.select(db,db.COLUMN_pos_product_name+"=\""+invoice.getProductName()+"\"").get(0).getTitle();
            }catch (Exception e){
