...
public class RegularInvoicePayment extends AppCompatActivity {
    DBInitialization db;
    String id="";
    String category="";
    Context context;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        actionBar.hide();
        setContentView(R.layout.activity_regular_invoice_payment);
        context = this;
        ...
        {
            posInvoiceHead = PosInvoiceHeadCombianRegular.getPosInvoiceCombianInvoiceWeb(PosInvoiceHeadRegularWeb.select(db,db.COLUMN_pos_invoice_head_id+"="+id)).get(0);
        }
        else
        {
        ...
        Button make_payment  = FontSettings.setTextFont((Button) findViewById(R.id.make_payment),this,db,"pos_sale_payment_popbox_invoice_done");
        ...
        make_payment.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                final String fpayment_mode = payment_mode;
                ...
                if(category.toUpperCase().equals("W"))
                {
                    final PosInvoiceHeadRegularWeb posInvoiceHead = PosInvoiceHeadRegularWeb.select(db,db.COLUMN_pos_invoice_head_id+"="+id).get(0);
                        ...
                        public void onClick(DialogInterface dialog, int which) {
                            switch (which){
                                case DialogInterface.BUTTON_POSITIVE:
                                    ...
                                    cashPaymentReceivedDetails.setVoucher_no("Rec-W-"+sl_id);
                                    ...
                                    hideSoftKeyboard(RegularInvoicePayment.this);
                                    dialog.dismiss();
                                    dialog.dismiss();
                                    progressDoalog_bank = new ProgressDialog(context);
                                    try {
                                    ...
                                    if(category.toUpperCase().equals("W")) {
                                        ...
                                        uploadData.data = PosInvoiceHeadCombianRegular.getPosInvoiceCombian(new ArrayList<PosInvoiceHeadRegular>(),PosInvoiceHeadRegularWeb.select(db,db.COLUMN_pos_invoice_head_id+"="+id));
                                    }
                                    else
                                    {
                                    ...
                                    uploadData.url=url_pos_invoice_upload + cSystemInfo.getUserDetails(cSystemInfo);
                    ...
                    AlertDialog.Builder builder = new AlertDialog.Builder(RegularInvoicePayment.this);
                ...
                {
                    final PosInvoiceHeadRegular posInvoiceHead = PosInvoiceHeadRegular.select(db,db.COLUMN_pos_invoice_head_invoice_id+"="+id).get(0);
                    posInvoiceHead.setTotal_paid_amount(posInvoiceHead.getTotal_amount());
                    cashPaymentReceive.setCard_type(cardT);
                    cashPaymentReceive.setBank_name(bank);
                    cashPaymentReceive.setCheque_no(chequeN);
                        ...
                        public void onClick(DialogInterface dialog, int which) {
                            switch (which){
                                case DialogInterface.BUTTON_POSITIVE:
                                    ...
                                    cashPaymentReceivedDetails.setAmount(cashPaymentReceive.getCash_amount()+cashPaymentReceive.getCard_amount());
                                    cashPaymentReceivedDetails.setDateTime(date_time);
                                    cashPaymentReceivedDetails.setVoucher_no("Rec-P-"+sl_id);
                                    cashPaymentReceivedDetails.insert(db);

                                    DebugHelper.print(cashPaymentReceivedDetails);
                                    PaymentReceiveVoucher paymentReceiveVoucher = new PaymentReceiveVoucher();
