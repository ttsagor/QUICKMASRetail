...
import static com.quickmas.salim.quickmasretail.Database.DBInitialization.TABLE_PosInvoice_regular_web;

public class PosInvoiceRegularWeb {
    public int id;
    ...
    {
        String con = COLUMN_pos_invoice_invoiceID_web+"="+this.getInvoiceID()+" AND "+COLUMN_pos_invoice_productSKU_web+"='"+this.getProductName()+"'";
        db.insertData(getContectValue(db), TABLE_PosInvoice_regular_web,con);
    }

    public void update(DBInitialization db)
    {
        db.updateData(getContectValue(db), TABLE_PosInvoice_regular_web,db.COLUMN_pos_invoice_id_web+"="+this.getId());
    }
    public static void update(DBInitialization db,String data,String con)
    {
        db.updateData(data,con, TABLE_PosInvoice_regular_web);
    }

    public static int count(DBInitialization db, String con)
    {
        return db. getDataCount(TABLE_PosInvoice_regular_web,con);
    }

    public static int sumData(DBInitialization db,String sel, String con)
    {
        return db.sumData(sel, TABLE_PosInvoice_regular_web, con);
    }

    public static ArrayList<PosInvoiceRegularWeb> getPendingInvoices(DBInitialization db)
    {
        return select(db, COLUMN_pos_invoice_status_web+"=0");
    }
    public static ArrayList<PosInvoiceRegularWeb> getPaymentPendingInvoices(DBInitialization db)
    {
        return select(db, COLUMN_pos_invoice_status_web+"=1 OR "+COLUMN_pos_invoice_status_web+"=0");
    }
    public static ArrayList<PosInvoiceRegularWeb> select(DBInitialization db, String con)
    {
        Cursor c = db.getData("*", TABLE_PosInvoice_regular_web,con,"");
        ArrayList<PosInvoiceRegularWeb> invoices = new ArrayList<PosInvoiceRegularWeb>();
        ...
        {
            Cursor c = db.getData("*",TABLE_PosInvoice_regular_web, COLUMN_pos_invoice_status_web+"!=0"+" OR "+ COLUMN_pos_invoice_status_web+"!=4", COLUMN_pos_invoice_invoiceID_web);
            return c.getCount();
        }

        public static void deletePendingInvoices(DBInitialization db,int id)
        {
            db.deleteData( TABLE_PosInvoice_regular_web ,COLUMN_pos_invoice_status_web+"=0 AND "+COLUMN_pos_invoice_invoiceID_web+"="+id);
        }
        ...
        {
            db.updateData(COLUMN_pos_invoice_status_web+"="+newS,COLUMN_pos_invoice_invoiceID_web+"="+String.valueOf(invoiceID), TABLE_PosInvoice_regular_web);
        }

        public static void updateInvoiceStatus(DBInitialization db,String newS)
        {
            db.updateData(COLUMN_pos_invoice_status_web+"="+newS,COLUMN_pos_invoice_status_web+"=0", TABLE_PosInvoice_regular_web);
        }

        public static void deleteIteamExchange(DBInitialization db)
        {
            db.deleteData( TABLE_PosInvoice_regular_web ,COLUMN_pos_invoice_status_web+"=9");
        }

        public static void deleteIteamFromPendingList(DBInitialization db,int invoiceID,int product)
        {
            db.deleteData( TABLE_PosInvoice_regular_web ,COLUMN_pos_invoice_status_web+"=0 AND "+COLUMN_pos_invoice_invoiceID_web+"="+invoiceID +" AND "+COLUMN_pos_invoice_productId_web+"="+product);
        }
        ...
        {
            return db.getMax(TABLE_PosInvoice_regular_web,COLUMN_pos_invoice_invoiceID_web, COLUMN_pos_invoice_status_web + "!=0");
        }
        public static int getNewInvoice(DBInitialization db)
        {
            return db.getMax(TABLE_PosInvoice_regular_web,COLUMN_pos_invoice_invoiceID_web,  "1=1");
        }
