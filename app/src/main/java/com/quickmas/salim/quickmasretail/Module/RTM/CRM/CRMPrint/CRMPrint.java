package com.quickmas.salim.quickmasretail.Module.RTM.CRM.CRMPrint;
...
import com.quickmas.salim.quickmasretail.Module.RTM.CRM.CRMPrint.Adaptar_crm_invoice_company;
...
public class CRMPrint extends AppCompatActivity {
    DBInitialization db;
    int amountCount=0;
    int uPriceCount=0;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        invoices = CRMInvoice.select(db,db.COLUMN_crm_Invoice_invoice_id+"="+in_invoice);

        CRMInvoiceHead crmInvoiceHead = CRMInvoiceHead.select(db,db.COLUMN_crm_Invoice_head_id+"="+in_invoice).get(0);


        String invoice = invoices.get(0).getPrefix() + String.valueOf(invoices.get(0).getInvoice_id());

        sell_invoice_outlet = setTextFont(sell_invoice_outlet, this, crmInvoiceHead.getOutlet());
        ...
        for(String img : crmInvoiceHead.getPhoto().split(";"))
        {
            image_list.add(Uri.parse(Environment.getExternalStorageDirectory().toString() + "/" + FileManagerSetting.folder_name + "/" + FileManagerSetting.rtm_crm_product + "/" + img.trim()));
        }
